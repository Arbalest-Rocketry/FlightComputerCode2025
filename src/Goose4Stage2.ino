    ...
    switch(stage_detector.stage){
      ...
      case 2:
        ...
        if((millis() - delayStart) >= 3000){
          ...
          //WAIT 3 SECONDS AFTER SEPARATION TO IGNITE
        }
        break;
      case 3:
        //IGNITED, WAIT TO CHECK IF IT IGNITED
        /*
        if(tiltLock(angleX,angleY)){
          cutoff();
          stage_detector.stage = 10;
        ...
        transmit_data(message,rf95);
        if(filtered_acceleration <= -8){
            //detect second stage ignition
            stage_detector.stage = 4;
        }
        break;
      case 4:
        //WAITING FOR SECOND STAGE BURNOUT
        if (millis() - lastSDWrite >= SDWriteInterval) {
          File dataFile = SD.open(filenameObj.c_str(), FILE_WRITE);
          logData(dataFile, acceleration, altitude, filtered_acceleration, filtered_altitude, speed, stage_detector.stage, running_time);
          dataFile.flush(); // Ensure data is written
        ...
        transmit_data(message,rf95);
        if(stage_detector.burnout(filtered_acceleration)){
            stage_detector.stage = 5;
        }
        break;
      case 5:
        //SECOND STAGE BURNOUT, WAIT FOR APOGEE
        if (millis() - lastSDWrite >= SDWriteInterval) {
          File dataFile = SD.open(filenameObj.c_str(), FILE_WRITE);
          logData(dataFile, acceleration, altitude, filtered_acceleration, filtered_altitude, speed, stage_detector.stage, running_time);
          dataFile.flush(); // Ensure data is written
        ...
        transmit_data(message,rf95);
        //&& (filtered_altitude >= 6339) if need here
        if((stage_detector.apogee(filtered_altitude))){
          //APOGEE REACHED
          deployS2drouge();//deploy first stage main cause we just have main
        ...
        break;
      case 6:
        //APOGEE REACHED AND DROUGE OPENED, WAIT FOR MAIN
        if (millis() - lastSDWrite >= SDWriteInterval) {
          File dataFile = SD.open(filenameObj.c_str(), FILE_WRITE);
          logData(dataFile, acceleration, altitude, filtered_acceleration, filtered_altitude, speed, stage_detector.stage, running_time);
          dataFile.flush(); // Ensure data is written
      ...
      case 8:
        ...
        break;
      case 9:
        //LOW POWER MODE, WAITING FOR RECOVERY
        transmit_data(message,rf95);
        tone(Buzzer, 2000);
        delay(1000);
        noTone(Buzzer);
